...
public:
    ...
    DCRgraph(SocialGraph *g, int communityId, int threshold, double commBenefit, double thresholdB,
    ...
    map<int, double>
    updateGainAndCurrentLiveAfterAddingNodeMig(int nodeId, vector<int> *currentLive, double *currentLiveB);
    ...
    void updateInitalGainMig(map<int, double> *mapInf, map<int, double> *mapInfB, map<int, int> *mapDead = nullptr,
                             map<int, double> *mapInfMaf = nullptr); // used to update intial Gain when new dcrGraph generated
    ...
    double commBenefit;
    double thresholdB;
    map<int, double> trackGainB; // store node id -> gain (int type) of that node to this dcr graph , USED FOR SANDWICH SOLUTION
